  <ul>
    	<li>
		    	<span id="{{ str_replace('/', '@',$directory)}}"> {{ ucfirst(str_limit(basename($directory),15)) }}</span>
...
<ul><li data-jstree='{"icon":"glyphicon glyphicon-leaf"}'><span id="{{ str_replace('/', '@',$sf)}}">{{ ucfirst(str_limit(basename($sf),15))}}</span></li></ul>
@endforeach
   @endif
    	</li>
</ul>
