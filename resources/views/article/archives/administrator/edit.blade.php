...
<form method="POST" action="{{ route('article-archives.update',['article'=>$archive] )}}"  enctype="multipart/form-data" class="">
	...
	<div>
		...
		<input type="text" name="gallery_photo" value="{{ $archive->gallery_photo }}" >
	</div>
	...
	<div>
		...
		<input type="text" name="stop_publication_at" class="datepicker" @if(isset($archive->start_publication_at)) value='{{ date("d-m-Y H:i:s", strtotime($archive->start_publication_at))}}' @endif autocomplete="off">
	</div>
